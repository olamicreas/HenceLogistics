...
// Reusable Back Header for Sub-screens
function SubScreenHeader({ title, onBack }: { title: string; onBack: () => void }) {
  return (
    <View style={localStyles.subHeader}>
      <TouchableOpacity onPress={onBack} style={localStyles.backButton} activeOpacity={0.7}>
        <Ionicons name="arrow-back" size={20} color="#0F1A14" />
      </TouchableOpacity>
      ...
      <ScrollView style={{ flex: 1, padding: 14 }}>
        ...
        <View style={localStyles.card}>
          <TouchableOpacity style={localStyles.supRow} onPress={() => Alert.alert("Live Chat", "Connecting you to an agent...")}>
            <View style={localStyles.supIco}><Ionicons name="chatbubbles" size={16} color="#1A7A4A" /></View>
          ...
          <TouchableOpacity style={localStyles.supRow} onPress={() => Linking.openURL('tel:+353830000000')}>
            <View style={localStyles.supIco}><Ionicons name="call" size={16} color="#1A7A4A" /></View>
            <View style={localStyles.supInfo}>
          ...
          <TouchableOpacity style={[localStyles.supRow, { borderBottomWidth: 0 }]} onPress={() => Linking.openURL('https://hence.com/help')}>
            <View style={localStyles.supIco}><Ionicons name="document-text" size={16} color="#1A7A4A" /></View>
            <View style={localStyles.supInfo}>
        ...
        <View style={localStyles.card}>
          <TouchableOpacity style={[localStyles.supRow, { borderBottomWidth: 0 }]} onPress={() => Linking.openURL('mailto:support@hencelogistics.com')}>
            <View style={[localStyles.supIco, { backgroundColor: '#FEF3E2', borderColor: '#D4860A' }]}>
              <Ionicons name="alert-circle" size={16} color="#D4860A" />
            </View>
            <View style={localStyles.supInfo}>
...
function SettingsScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F4F8F6' }}>
      <SubScreenHeader title="Account Settings" onBack={onBack} />
      <ScrollView style={{ flex: 1, padding: 14 }}>
        <Text style={[localStyles.sdiv, { marginTop: 4 }]}>Notifications</Text>
        <View style={localStyles.card}>
...
function DeleteScreen({ logout, onBack }: { logout: () => void; onBack: () => void }) {
  const [mode, setMode] = useState<'deactivate' | 'delete'>('deactivate');
  const [confirmText, setConfirmText] = useState('');

  return (
    <View style={{ flex: 1, backgroundColor: '#F4F8F6' }}>
      <SubScreenHeader title="Account Deactivation" onBack={onBack} />
      <ScrollView style={{ flex: 1, padding: 14 }}>
        <View style={{ alignItems: 'center', paddingVertical: 16 }}>
          <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: '#FDEDEC', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
            <Ionicons name="alert-circle" size={24} color="#C0392B" />
          </View>
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#0F1A14' }}>This affects your account</Text>
          <Text style={{ fontSize: 12, color: '#7A9080', marginTop: 4, lineHeight: 18, textAlign: 'center' }}>
        ...
        <View style={[localStyles.card, { padding: 12 }]}>
          <TouchableOpacity style={{ flexDirection: 'row', gap: 10, paddingBottom: 12, borderBottomWidth: 1, borderColor: '#D4E2DA' }} onPress={() => setMode('deactivate')}>
            <Ionicons name={mode === 'deactivate' ? "radio-button-on" : "radio-button-off"} size={18} color={mode === 'deactivate' ? '#1A7A4A' : '#B8CEC3'} />
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 13, fontWeight: '500', color: '#0F1A14' }}>Deactivate my account</Text>
              <Text style={{ fontSize: 11, color: '#7A9080', marginTop: 2 }}>Temporary — hides your profile and pauses bookings. You can log back in anytime to reactivate.</Text>
            </View>
          </TouchableOpacity>

          <TouchableOpacity style={{ flexDirection: 'row', gap: 10, paddingTop: 12 }} onPress={() => setMode('delete')}>
            <Ionicons name={mode === 'delete' ? "radio-button-on" : "radio-button-off"} size={18} color={mode === 'delete' ? '#C0392B' : '#B8CEC3'} />
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 13, fontWeight: '500', color: '#C0392B' }}>Delete my account permanently</Text>
              <Text style={{ fontSize: 11, color: '#7A9080', marginTop: 2 }}>Removes your profile, order history and saved details. This cannot be reversed.</Text>
            </View>
          </TouchableOpacity>
        </View>

        <View style={{ marginTop: 14 }}>
          <Text style={{ fontSize: 10, fontWeight: '600', color: '#3D5046', textTransform: 'uppercase', marginBottom: 5 }}>Type DELETE to confirm</Text>
          <TextInput 
            style={{ width: '100%', padding: 12, borderWidth: 1, borderColor: '#D4E2DA', borderRadius: 8, fontSize: 13, color: '#0F1A14', backgroundColor: '#fff' }} 
            placeholder="DELETE" 
            value={confirmText} 
        ...
        <TouchableOpacity 
          style={{ width: '100%', marginTop: 14, padding: 13, backgroundColor: '#1A7A4A', borderRadius: 10, alignItems: 'center' }} 
          onPress={() => {
...
export default function AppRoot() {
  const { token, currentScreen, user, logout, setCurrentScreen } = useAppContext();
...
const localStyles = StyleSheet.create({
  subHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#D4E2DA' },
  backButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#F4F8F6', alignItems: 'center', justifyContent: 'center' },
  subHeaderTitle: { fontSize: 15, fontWeight: '600', color: '#0F1A14' },
  sdiv: { fontSize: 10, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, color: '#7A9080', marginVertical: 10 },
  ...
  supIco: { width: 34, height: 34, borderRadius: 9, backgroundColor: '#E8F5EE', borderWidth: 1, borderColor: '#C5E8D4', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  supInfo: { flex: 1 },
  supTitle: { fontSize: 13, fontWeight: '500', color: '#0F1A14' },
